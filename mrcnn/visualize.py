...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        contours = find_con tours(padded_mask, 0.5)
...
def drawinstances(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16), ax=None):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    ...
    for i in range(N):
        ...
        cv2.putText(masked_image, caption,
                   (x1, yyy), cv2.FONT_HERSHEY_SIMPLEX, 1.5, 
                   [int(x*255) for x in (color)],4)
        # Mask
        mask = masks[:, :, i]
        masked_image = apply_mask(masked_image, mask, color)
